    ...
    def generate(
        ...
        Args:
            ...
            bos_token (int, optional): beginning of sentence token
                (default: self.eos)
        """
        return self._generate(models, sample, **kwargs) #models arg by ayush

    def _generate(
        self,
        models, #added by ayush
        sample: Dict[str, Dict[str, Tensor]],
        prefix_tokens: Optional[Tensor] = None,
        ...
        model = EnsembleModel(models) # added model argument by AYush
        encoder_input = { # added by ayush
...
class EnsembleModel(nn.Module):
    ...
    @torch.jit.export
    def forward_encoder(self, encoder_input): #added by ayush
        if not self.has_encoder():
            return None
        # return [model.encoder.forward_torchscript(net_input) for model in self.models]
        src_tokens=encoder_input['src_tokens']
        src_lengths=encoder_input['src_lengths']
        # return [model.encoder(src_tokens, src_lengths) for model in self.models]
        return [model.encoder.forward_torchscript(src_tokens, src_lengths, decoder=model.decoder) for model in self.models] #added by ayush
